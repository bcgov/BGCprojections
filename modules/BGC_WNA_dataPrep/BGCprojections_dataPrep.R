...
defineModule(sim, list(
  ...
  parameters = bindrows(
    ...
    defineParameter("climrVars", "character", 
                    c("DD5", "DD_0_at", "DD_0_wt", "PPT05", "PPT06", "PPT07", 
                      "PPT08", "PPT09", "CMD", "PPT_at", "PPT_wt", "CMD07", 
                      "SHM", "AHM", "NFFD", "PAS", "CMI"), 
                    NA, NA,
                    paste("Climate variables to obtain from `climr::climr_downscale`.",
                          "May include variables listed in `P(sim)$climPredictors` or variables",
                          "necessary to produce them (using `addVars()`). See `climr::list_variables()`." )),
    defineParameter("climPredictors", "character", 
                    ...
                    paste("Climate covariares used to fit BGC zone ~ climate model. May include variables",
                          "and others derived from them using `addVars()`. See `climr::list_variables()`",
                          "and `?addVars()` for options.")),
    defineParameter("GCMs", "character", list_gcm(), NA, NA,
                    paste("List of General Circulation Models (GCMs) to obtain climate projections for",
                          "`sim$predData`.")),
    defineParameter("GCMperiods", "character", list_gcm_period(), NA, NA,
    ...
    defineParameter("SSPs", "character", list_ssp(), NA, NA,
    ...
    defineParameter("nthread", "integer", 1L, 1L, Inf,
                    paste("Passed to `climr_downscale(..., nthread)` to parallelise climate downscaling operations")),
